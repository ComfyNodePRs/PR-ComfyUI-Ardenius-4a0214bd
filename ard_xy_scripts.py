...
class ARD_XY_SCRIPTS:
    ...
    DESCRIPTION = "ARD XY Tester is for testing images using a range of steps, cfg, or denoise in ComfyUI."
    # my_code_start
    def ard_xy_scripts(self, activate, width, height, start_cfg, end_cfg, step_cfg, start_steps, end_steps, step_steps, start_denoise, end_denoise, step_denoise, seed):
        ...
        if activate == 'On':
            ...
            json_file = os.path.join(node_dir, "ard_data/xy_data.json")

            if os.path.exists(json_file):
                xy_dict = ard_lib.read_dict_from_json(json_file)
                ...
                ard_lib.save_dict_to_json(xy_dict, json_file)
            else:
                xy_dict = xy_dict_in
                ard_lib.save_dict_to_json(xy_dict_in, json_file)
                cfg = xy_dict['xy_current']['cfg']
                steps = xy_dict['xy_current']['steps']
                denoise = xy_dict['xy_current']['denoise']
            ...
            steps = 20
            denoise = 8
            print('\n---\nARD XY Script is off and set to defaults:\nsteps: 20\ncfg: 8\ndenoise: 1\n---\n')

        return width, height, cfg, steps, denoise, seed
    ...
    def update_steps(s, xy_dict):
        if int(xy_dict['xy_current']['steps']) < int(xy_dict['xy_input']['end_steps']):
            ...
            print(f"\n---\nARD XY Script : executing step {xy_dict['xy_current']['steps']}\n---\n")
        else:
        ...
        if float(xy_dict['xy_current']['cfg']) < float(xy_dict['xy_input']['end_cfg']):
            ...
            print(f"\n---\nARD XY Script : executing cfg {xy_dict['xy_current']['cfg']}\n---\n")
        else:
        ...
        if float(xy_dict['xy_current']['denoise']) < float(xy_dict['xy_input']['end_denoise']):
            ...
            print(f"\n---\nARD XY Script : executing denoise {xy_dict['xy_current']['denoise']}\n---\n")
        else:
